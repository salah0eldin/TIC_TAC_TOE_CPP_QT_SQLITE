...
int DatabaseManager::signUp(const QString &username, const QString &password)
{
    ...
    if (!selectQuery.exec()) {
        // Print error message if query execution fails
        qDebug() << "Error executing select user up query:" << selectQuery.lastError().text();
    ...
    if (!insertQuery.exec()) {
        // Print error message if query execution fails
        qDebug() << "Error executing insert user query:" << insertQuery.lastError().text();
        return DATABASE_ERROR; // Return error code
    }
...
bool DatabaseManager::saveSession(const int &userid, const QString &against, const int &wins,
                                  const int &losses, const int &ties)
{
    // Prepare SQL query to insert new session
    QSqlQuery insertQuery;
    insertQuery.prepare("INSERT INTO sessions (userid, against, wins, losses, ties) "
                        "VALUES (?, ?, ?, ?, ?)");
...
bool DatabaseManager::saveGame(const int &session_id, const char &playerCharacter,
                               const QString &moves, const QString &state)
{
    // Prepare SQL query to insert a new game
    QSqlQuery insertQuery;
    insertQuery.prepare("INSERT INTO games (session_id, playerCharacter, moves, state) "
                        "VALUES (?, ?, ?, ?)");
